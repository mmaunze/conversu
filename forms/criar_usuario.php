...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($rowCheckNome['total'] > 0) {
        ...
        $sql = "INSERT INTO utilizador (nome, id_tipo_utilizador, username, email, senha) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("sisss", $nome, $id_tipo_utilizador, $username, $email, $username);
            if ($stmt->execute()) {
                $response['status'] = 'success';
                $response['message'] = 'Usuario cadastrado com sucesso!';
            } else {
                logError("Erro ao cadastrar usuario: " . $stmt->error);
                $response['status'] = 'error';
                $response['message'] = 'Houve um problema ao cadastrar o usuario. Tente novamente mais tarde.';
            }
            $stmt->close();
        } else {
